...
char
    g_szNotifySound[PLATFORM_MAX_PATH],
    g_szCountdownSound[PLATFORM_MAX_PATH],
    g_szLiveSound[PLATFORM_MAX_PATH]
;
...
public void OnMapStart()
{
    for (int iClient = 1; iClient <= MaxClients; iClient ++)
    {
        ...
        g_iClientCommandSpamAttempts[iClient] = 0;
    }

    LoadSoundFromCvar(g_cvSoundNotify, g_szNotifySound, sizeof g_szNotifySound, DEFAULT_NOTIFY_SOUND);
    LoadSoundFromCvar(g_cvSoundCountdown, g_szCountdownSound, sizeof g_szCountdownSound, DEFAULT_COUNTDOWN_SOUND);
    LoadSoundFromCvar(g_cvSoundLive, g_szLiveSound, sizeof g_szLiveSound, DEFAULT_LIVE_SOUND);

    PrecacheSound(g_szNotifySound);
    PrecacheSound(g_szCountdownSound);
    PrecacheSound(g_szLiveSound);
}
...
/**
 * Checks if all players on a specific team are ready.
 *
 * @param iTeam         Team index (TEAM_SURVIVOR or TEAM_INFECTED)
 * @return              True if all players on the team are ready and the team meets
 *                      the minimum size requirement (if strict mode is enabled),
 *                      false otherwise
 */
...
/**
 * Sets the ready status for all players on a specific team.
 *
 * @param iTeam         Team index (TEAM_SURVIVOR or TEAM_INFECTED)
 * @param bReady        True to set all players as ready, false to set all as not ready
 * @return              True if all players were set successfully, false if strict mode is enabled
 *                      and not enough players were changed
 */
...
/**
 * Checks if both teams are ready to start the game.
 *
 * @return              True if both infected and survivor teams are ready, false otherwise
 */
bool IsGameReady() {
...
/**
 * Checks if the ReadyUp panel is visible for a client.
 *
 * @param iClient       Client index
 * @return              True if the ReadyUp panel is visible, false otherwise
 */
...
/**
 * Sets the visibility of the ReadyUp panel for a client.
 *
 * @param iClient       Client index
...
/**
 * Executes the forward when a client changes their ready status.
 *
 * @param iClient       Client index
 * @param bReady        True if client is ready, false otherwise
 * @noreturn
 */
...
/**
 * Executes the forward when preparing a ReadyUp panel item.
 *
...
/**
 * Executes the forward when a ReadyUp panel item is removed.
 *
 * @param ePos          Panel position
 * @param iOldIndex     Old item index before removal
 * @param iNewIndex     New item index after removal
 * @noreturn
 */
...
/**
 * Plays the live sound to all clients.
 *
 * @noreturn
 */
void PlayLiveSound()
{
    if (IsSoundEnabled()) {
        EmitSoundToAll(g_szLiveSound, .volume = 0.5);
    }
}

/**
 * Plays the countdown sound to all clients.
 *
 * @noreturn
 */
...
{
    if (IsSoundEnabled()) {
        EmitSoundToAll(g_szCountdownSound, .volume = 0.5);
    }
}

/**
 * Plays the notification sound to a specific client.
 *
 * @param iClient       Client index
 * @noreturn
 */
...
{
    if (IsSoundEnabled()) {
        EmitSoundToClient(iClient, g_szNotifySound);
    }
}

/**
 * Checks if sounds are enabled via cvar.
 *
 ...
 *
 * @param sSoundPath    Sound file path relative to sound/ folder
...
/**
 * Loads a sound path from a cvar with a fallback default value.
 * If the cvar value is empty or the sound file doesn't exist,
 * the default value is used instead.
 *
 * @param cv            ConVar handle to read from
 * @param szBuffer      Buffer to store the sound path
 * @param iLength       Maximum length of the buffer
 * @param szDefault     Default sound path if cvar value is invalid
 * @noreturn
 */
void LoadSoundFromCvar(ConVar cv, char[] szBuffer, int iLength, const char[] szDefault)
{
    GetConVarString(cv, szBuffer, iLength);
...
/**
 * Gets a client's name and truncates it if it exceeds the maximum size.
 *
 * @param iClient       Client index
 * @param szName        Buffer to store the client name
 * @param length        Maximum length of the buffer
 * @param iMaxSize      Maximum visible size before truncation with "..."
 * @noreturn
 */
void GetClientNameFixed(int iClient, char[] szName, int length, int iMaxSize)
{
    GetClientName(iClient, szName, length);
